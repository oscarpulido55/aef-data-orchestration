...
class WorkflowsGenerator:
    def __init__(self, workflow_config, exec_config, generate_for_pipeline):
    ...
    def load_templates(self):
        ...
        self.workflow_template = read_template(
            "workflow", self.generate_for_pipeline, self.workflows_folder, "json"
        )
        self.level_template = read_template(
            "level", self.generate_for_pipeline, self.workflows_folder, "json"
        )
        self.thread_template = read_template(
            "thread", self.generate_for_pipeline, self.workflows_folder, "json"
        )
        self.cloud_function_async_template = read_template(
            "async_call", self.generate_for_pipeline, self.workflows_folder, "json"
        )
    ...
    def process_levels(self, config):
        ...
        for index, level in enumerate(config):
            ...
            if len(threads) == 1:
                ...
                level_body = self.level_template.replace(
    ...
    def process_threads(self, threads, level_id):
        ...
        for index, thread in enumerate(threads):
            ...
            steps = self.process_steps(
                thread.get("STEPS"), level_id, thread.get("THREAD_ID"), single_thread
            )
    ...
    def process_steps(self, steps, level_id, thread_id, single_thread):
        ...
        step_bodies = []
        cloud_function_intermediate_name = self.exec_config.get(
            "function_intermediate_name"
        )
        jobs_definitions_bucket = f"{self.exec_config.get('data_transformation_project')}_{self.exec_config.get('jobs_definition_bucket_suffix')}"

        for index, step in enumerate(steps):
            step_body = ""
            if step.get("TYPE") == "sync":
                step_body = self.process_step_sync(
                    assemble_cloud_function_id(
                        cloud_function_intermediate_name, self.exec_config
                    ),
                    step,
                    step.get("FUNCTION_NAME"),
                )
            elif step.get("TYPE") == "async":
                step_body = self.process_step_async(
                    level_id,
                    assemble_cloud_function_id(
                        cloud_function_intermediate_name, self.exec_config
                    ),
            ...
            step_body = self.process_next_step(
                steps, step, index, level_id, thread_id, step_body, single_thread
            )
    ...
    def process_step_sync(self, cloud_function_level_1_id, step, cloud_function_name):
        ...
        if "CONTINUE_IF_FAIL" in step.keys():
            step_body = step_body.replace(
                "{CONTINUE_IF_FAIL_BLOCK}", ',"PcontinueIfFail": "True"'
            )
        else:
    ...
    def process_step_async(
        ...
        step_body = step_body.replace(
            "{CLOUD_FUNCTION_ID_TO_INVOKE}",
            assemble_cloud_function_id(FUNCTION_ID_NAME, self.exec_config),
        )
        step_body = step_body.replace(
            "{CLOUD_FUNCTION_STATUS_TO_INVOKE}",
            assemble_cloud_function_id(FUNCTION_STATUS_NAME, self.exec_config),
        )
        step_body = step_body.replace("{JOB_IDENTIFIER}", step.get("JOB_ID"))
        step_body = step_body.replace("{JOB_NAME}", step.get("JOB_NAME"))
        step_body = step_body.replace(
            "{WAIT_TIME_SECONDS}", step.get("WAIT_TIME_SECONDS")
        )
        step_body = step_body.replace(
            "{WAIT_TIME_SECONDS}", step.get("WAIT_TIME_SECONDS")
        )
        ...
        step_body = step_body.replace(
            "{ASYNC_JOB_STATUS_VARIABLE_NAME}", step.get("JOB_ID") + "_async_job_status"
        )
        if "READ_INPUT_FROM" in step.keys():
            step_body = step_body.replace(
                "{READ_INPUT_FROM}", step.get("READ_INPUT_FROM")
            )
        else:
        ...
        if "ASYNC_TIMEOUT_LOOP_IN_MINUTES" in step.keys():
            step_body = step_body.replace(
                ...
                ',"PtimeoutMinutes": ' + step.get("ASYNC_TIMEOUT_LOOP_IN_MINUTES"),
            )
        else:
    ...
    def process_next_step(
        self, steps, step, index, level_id, thread_id, step_body, single_thread
    ):
        ...
        if step.get("TYPE") in ("sync", "async", "workflows"):
            if "NEXT" not in step.keys():
                try:
                    ...
                    if (
                        level_exists(int(level_id) + 1, self.workflow_config)
                        and single_thread
                    ):
                        if level_exists_and_is_parallel(
                            int(level_id) + 1, self.workflow_config
                        ):
                            step_body = step_body.replace(
                                "{NEXT_JOB_ID}", "Level_" + str(int(level_id) + 1)
                            )
                        else:
                            step_body = step_body.replace(
                                "{NEXT_JOB_ID}",
