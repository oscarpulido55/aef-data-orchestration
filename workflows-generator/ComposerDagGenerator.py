...
class ComposerDagGenerator:
    ...
    def load_templates(self):
        ...
        self.workflow_template = read_template(
            "workflow", self.generate_for_pipeline, "composer-templates", "py"
        )
        self.level_template = read_template(
            "level", self.generate_for_pipeline, "composer-templates", "py"
        )
        self.thread_template = read_template(
            "thread", self.generate_for_pipeline, "composer-templates", "py"
        )
        # add new templates for other executors here
        self.dataform_tag_executor_template = read_template(
            "dataform_tag_executor",
            self.generate_for_pipeline,
            "composer-templates",
            "py",
        )
        self.dataflow_flextemplate_job_executor_template = read_template(
            "dataflow_flextemplate_job_executor",
            self.generate_for_pipeline,
            "composer-templates",
            "py",
        )
        self.dataproc_serverless_job_executor_template = read_template(
            "dataproc_serverless_job_executor",
            self.generate_for_pipeline,
            "composer-templates",
            "py",
        )
    ...
    def generate_workflows_body(self):
        ...
        workflow_body = workflow_body.replace(
            "<<STEPS_ARGS>>", self.process_steps_vars(self.workflow_config)
        )
    ...
    def process_levels(self, config):
        ...
        for index, level in enumerate(config):
            ...
            level_body = level_body.replace(
                "<<THREAD_DEPENDENCIES>>",
                self.get_thread_dependency_string(
                    level.get("THREADS"), level.get("LEVEL_ID")
                ),
            )
    ...
    def process_threads(self, threads, level_id):
        ...
        for index, thread in enumerate(threads):
            ...
            steps = self.process_steps(
                thread.get("STEPS"), level_id, thread.get("THREAD_ID")
            )
    ...
    def process_step_async(self, level_id, thread_id, step):
        ...
        if "dataform-tag-executor" in step.get("COMPOSER_STEP"):
            step_body = self.dataform_tag_executor_template.replace(
                "{JOB_ID}", step_name
            )
        if "dataflow-flextemplate-job-executor" in step.get("COMPOSER_STEP"):
            step_body = self.dataflow_flextemplate_job_executor_template.replace(
                "{JOB_ID}", step_name
            )
        if "dataproc-serverless-job-executor" in step.get("COMPOSER_STEP"):
            step_body = self.dataproc_serverless_job_executor_template.replace(
                "{JOB_ID}", step_name
            )
        step_body = step_body.replace("{LEVEL_ID}", level_id)
        step_body = step_body.replace("{THREAD_ID}", thread_id)
